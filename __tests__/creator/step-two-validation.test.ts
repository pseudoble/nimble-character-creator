...
import { validateStepTwo, getValidAncestryIds, getValidBackgroundIds } from "@/lib/creator/step-two-validation";
...
function makeDraft(overrides: Partial<CreatorDraft["stepTwo"]> = {}): CreatorDraft {
  return {
...
describe("Step 2 validation", () => {
  it("passes for valid payload", () => {
    const result = validateStepTwo(makeDraft(), validAncestryIds, validBackgroundIds);
    expect(result.valid).toBe(true);
    expect(result.errors).toEqual({});
  });

  it("fails when ancestryId is empty", () => {
    const result = validateStepTwo(makeDraft({ ancestryId: "" }), validAncestryIds, validBackgroundIds);
    expect(result.valid).toBe(false);
    expect(result.errors.ancestryId).toBeDefined();
  });

  it("fails when backgroundId is empty", () => {
    const result = validateStepTwo(makeDraft({ backgroundId: "" }), validAncestryIds, validBackgroundIds);
    expect(result.valid).toBe(false);
    expect(result.errors.backgroundId).toBeDefined();
  });

  it("fails when ancestryId is not in valid list", () => {
    const result = validateStepTwo(makeDraft({ ancestryId: "centaur" }), validAncestryIds, validBackgroundIds);
    expect(result.valid).toBe(false);
    expect(result.errors.ancestryId).toBeDefined();
  });

  it("fails when backgroundId is not in valid list", () => {
    const result = validateStepTwo(makeDraft({ backgroundId: "pirate" }), validAncestryIds, validBackgroundIds);
    expect(result.valid).toBe(false);
    expect(result.errors.backgroundId).toBeDefined();
  });
  ...
  it("passes when motivation is empty", () => {
    const result = validateStepTwo(makeDraft({ motivation: "" }), validAncestryIds, validBackgroundIds);
    expect(result.valid).toBe(true);
  });
...
describe("Step 2 ID lists match core data", () => {
